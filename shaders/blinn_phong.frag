...
layout(location = 7) in mat4 i_ViewMat;
layout(location = 11) in mat3 i_TBN;
...
void main() {
  ...
  for (int i = 0; i < lights.lightCount; i++) {
    ...
    if (light.position.w == 1.0f) {
      lightAccumulation += directional_light(light, normal, normalize(-i_FragPos), i_FragPos);
    } else if (light.position.w == 2.0f) {
      lightAccumulation += point_light(light, normal, normalize(-i_FragPos), i_FragPos);
    }
  }
