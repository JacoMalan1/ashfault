...
std::optional<std::shared_ptr<VulkanShader>> ShaderManager::get_vertex_shader(
    const std::string &name) {
  if (!this->m_VertexShaders.count(name)) return {};

  return this->m_VertexShaders[name];
}

std::optional<std::shared_ptr<VulkanShader>> ShaderManager::get_fragment_shader(
    const std::string &name) {
  if (!this->m_FragmentShaders.count(name)) return {};
